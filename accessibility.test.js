...
const tests = [
  ...
  ["keyboard focus remains visible on controls, room input, and canvas", () => {
    ...
    assert.match(duelStyles, /\.activity-context-actions a:focus-visible/);
  }],
  ["Discord Activity users can reach both policy documents", () => {
    // The policies sit in the Activity strip, which is shown whenever the
    // Activity is (the site header is hidden there).
    const strip = duelHtml.match(/<nav class="activity-context-actions"[^>]*>([^]*?)<\/nav>/)?.[1] || "";
    assert.match(strip, /<a class="activity-context-legal" href="\/terms\.html" target="_blank" rel="noopener noreferrer">TERMS<\/a>/);
    assert.match(strip, /<a class="activity-context-legal" href="\/privacy\.html" target="_blank" rel="noopener noreferrer">PRIVACY<\/a>/);
  }],
