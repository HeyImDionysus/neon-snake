...
The live-room adapter uses adaptive same-origin short polling against `/api/room`. One transport-owned clock starts quickly while a room is changing, backs a one-player wait from 700 ms to a 2 second presence refresh, accelerates when the second player arrives, and stays below one 138 ms game tick for the active guest. Host snapshots, guest turns, Ready changes, and countdown messages interrupt that clock and send immediately. The page does not run a second presence heartbeat.

Each request executes one atomic Redis Lua command that cleans stale slots, refreshes presence, applies the sender's allowed update, and returns the latest room state. The active host naturally publishes once per game tick instead of polling independently between snapshots. This keeps the dependency-free Vercel/Redis boundary while reducing idle room traffic by up to 70% and active host traffic by roughly 42% compared with the original fixed 600 ms / 80 ms cadence. Retry and rate-limit backoff always outrank the normal game cadence.
