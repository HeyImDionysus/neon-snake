...
Signal Codes also name live rooms. `PUBLIC LIVE ROOM` supports the two-seat Duel and four-seat Arena modes, with an ordered server-owned waiting line in one Vercel WebSocket room. Seat 1 chooses the mode before a round; switching modes atomically grows or shrinks the active seats, clears Ready state, and promotes or queues participants without client-controlled positions. Duel remains the 30 × 30 first-crash match. Arena uses a 60 × 60 board, one food cell per seat, and continues until one snake remains alive. The winner keeps their seat; losers rotate in elimination order and the queue head fills available seats. Waiting participants can watch but are never auto-ready or auto-started. Every seated player publishes bounded direction inputs over WebSocket; the browser never publishes authoritative state. The seat-0 Vercel Function owns the 138 ms simulation, resolves the appropriate two- or N-snake rules once, and broadcasts one verified slot-indexed snapshot through the existing Redis event relay to every Function instance and screen. Countdown requires at least two connected seated players and every seated player Ready. A disconnect, missed heartbeat, replaced connection, or revoked Ready state cancels the round immediately. Only completed two-account Duel rounds affect the leaderboard; Arena rounds are never recorded.
...
- **Public Live Room:** a six-character room Signal supports Duel (2 seats, 30 × 30) and Arena (up to 4 seats, 60 × 60). Seat 1 selects the mode, at least two seated players must be connected and Ready, and all additional visitors remain in the ordered waiting line.
- **Arena rotation:** the last surviving snake wins; eliminated snakes disappear immediately, losers rotate in elimination order, and the queue head fills seats without auto-ready or auto-start.
