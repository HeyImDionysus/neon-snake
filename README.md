...
- **End run:** every solo run and Autopilot run can be left at any time from the control deck or with `Escape`, returning to the mode choice without recording an attempt.
...
`wallpaper/android` is a native Android live wallpaper for Android 8.0 and newer. It runs a lightweight native engine rather than the game's rules: the snake follows a fixed serpentine route and eats what it passes, rendered with the same layered snake, face, Signals, Cores, pickup burst, and score HUD as the browser surface. Opening the installed app launches the system live-wallpaper chooser. The service has no `INTERNET` permission, stops its frame callbacks whenever the wallpaper is hidden, and lowers rendering from roughly 24 fps to 15 fps in system power-save mode. Every push builds a debug APK to prove the project compiles. Releases are built by `.github/workflows/release.yml` when a `v*` tag matching the `package.json` version is pushed: it signs the APK with the one release keystore held as `ANDROID_KEYSTORE`, `ANDROID_KEYSTORE_PASSWORD`, `ANDROID_KEY_ALIAS` and `ANDROID_KEY_PASSWORD` secrets in the protected `release` environment, optionally checks the certificate against the `ANDROID_CERT_SHA256` repository variable, and publishes both wallpapers with a `SHA256SUMS` file as a GitHub Release. The release fails rather than shipping an unsigned or debug-signed APK. Builds published before this workflow (up to v1.1.1) were debug-signed with different keys, so players must uninstall them before installing a release build.
