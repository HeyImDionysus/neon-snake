...
function runConsistencyMatrix() {
  ...
  for (let run = 0; run < 12; run += 1) {
    ...
    for (; ticks < 180 && !reference.over; ticks += 1) {
      ...
      // Every fourth run drives the human into a wall once the board has opened
      // up, so crash resolution, the winner and the terminal snapshot are
      // compared between the two implementations rather than only survival.
      const reckless = run % 4 === 3 && ticks >= 40;
      const moves = {
        [aiRole]: selected.direction,
        [humanRole]: reckless
          ? { ...reference[`${humanRole}Direction`] }
          : policyMove(reference, humanRole, aiRole, selected.direction),
      };
  ...
  assert.ok(
    completed.filter((entry) => entry.over).length >= 3,
    `Expected terminal rounds so crash, winner and final snapshot are compared: ${JSON.stringify(completed)}`,
  );
